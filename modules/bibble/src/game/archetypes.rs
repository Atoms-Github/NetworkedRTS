...
pub fn new_commander(owner: GlobalEntityID, race: RaceID) -> PendingEntity{
    PendingEntity::new3(
        OwnsResourcesComp::<CommanderProperty>::default(),
        TechTreeComp{ tree: GameData::gen_game_data() },
        CommanderComp{ alive: true, race },
    )
}
pub fn new_scene_manager() -> PendingEntity{
    PendingEntity::new2(
        SceneManager{ current: RtsSceneType::None, next: RtsSceneType::Lobby, completed_rounds: 0 },
        ScenePersistent{ keep_alive: true },
    )
}
